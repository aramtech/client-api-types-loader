...
const utilsConfig: typeof import("./utils.json") = JSON.parse(fs.readFileSync(path.join(currentDir, "./utils.json"), "utf-8"))
...
console.log(packageDotJson["api-types"])
...
program
    ...
    .action(async ({ scope, api_prefix, assets_prefix, base_url }: { [key: string]: string }) => {
        if(!packageDotJson["api-types"]){
            console.error("Please provide api types loading config in package.json before loading api types as in ", `{
                "api-prefix": string;
                "assets-prefix": string;
                "baseUrl": string; 
                scope?: string;
        ...
        const apiTypesDirFullPath = path.join(currentDir,"../api-types")
        ...
        const build_types = async () => {
            ...
            const content = [
                `// @ts-nocheck
import { $Enums, Prisma } from "@/../api-types/client/index.js";
